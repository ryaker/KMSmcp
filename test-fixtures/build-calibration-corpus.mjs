...
async function main() {
  ...
  // Deterministic in place of $sample (issue #63 finding #2): $sample re-runs
  // to a different fixture every time, and the calibration thresholds depend
  // on the fixture — a non-reproducible corpus makes recalibration unverifiable.
  // Sort by the stable `id` field and take a fixed-size prefix instead.
  ...
  // Include metadata.supersedes so we can tell an actual supersede chain
  // apart from two unrelated entries that merely share a subject facet
  // (issue #63 finding #3 — sharing a subject does not imply duplication;
  // e.g. sequential-but-distinct observations on the same topic).
  const sameSubjectDups = await coll.aggregate([
    { $match: {
        'metadata.subject': { $exists: true, $ne: null },
        $or: [{ flag: null }, { flag: { $exists: false } }],
      } },
    { $project: { id: 1, content: 1, contentType: 1, source: 1, 'metadata.subject': 1, 'metadata.supersedes': 1, _id: 0 } },
    { $group: { _id: '$metadata.subject', entries: { $push: '$$ROOT' }, n: { $sum: 1 } } },
    { $match: { n: { $gte: 2 } } },
  ]).toArray()
  let sameSubjectSkipped = 0
  for (const grp of sameSubjectDups) {
    if (grp.entries.length < 2) continue
    // Only auto-promote a pair actually linked by a supersede chain — do not
    // assume the first two entries in the group are duplicates just because
    // they share a subject facet.
    let a, b
    outer:
    for (const x of grp.entries) {
      for (const y of grp.entries) {
        if (x.id === y.id) continue
        if (x.metadata?.supersedes === y.id || y.metadata?.supersedes === x.id) {
          a = x; b = y
          break outer
        }
      }
    ...
    dupPairs.push({
      ...
      content_b: b.content,
      human_label: 'duplicate',
      rationale: `Entries linked by an explicit supersede chain (metadata.supersedes) under metadata.subject="${grp._id}" — supersession, not merely a shared subject facet, is the duplicate signal.`,
      provenance: 'same_subject_facet_supersede_chain',
    })
  }
  if (sameSubjectSkipped > 0) {
    console.log(`  (skipped ${sameSubjectSkipped} same-subject group(s) with no supersede chain between any pair — not auto-labeled as duplicate)`)
  }
  ...
  const fixture = {
    ...
    notes: [
      ...
      'duplicate_pairs sources: (1) SUPERSEDE chains [provenance=supersede_chain], (2) embedding-clustered candidates [embedding_cluster_high], (3) same-subject pairs linked by an explicit supersede chain [same_subject_facet_supersede_chain]',

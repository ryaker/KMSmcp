...
export class SparrowDBStorage implements StorageSystem {
  ...
  async storeEmbedding(
    ...
    if (this.internalIdMapLoaded) {
      try {
        const res = this.db.execute(
          `MATCH (k:${SparrowDBStorage.VECTOR_LABEL} {id: ${cypherStr(id)}}) RETURN id(k) AS nid`
        )
        const nid = res.rows[0]?.['nid']
        if (nid !== null && nid !== undefined) {
  ...
  private internalIdToUuid: Map<string, string> = new Map()
  /** True once the internalIdToUuid map has been bulk-loaded from the graph. */
  private internalIdMapLoaded = false
  ...
  private _ensureInternalIdMap(): void {
    ...
    try {
      ...
      logger.debug(
        `✅ SparrowDB internalId→UUID map loaded: ${this.internalIdToUuid.size} entries`
      )
    } catch (e) {
      logger.warn(
        `⚠️ SparrowDB internalId→UUID map load failed (vector results may be unresolvable): ${e instanceof Error ? e.message : String(e)}`
      )
  ...
  async findSimilar(
    ...
    if (embedding.length !== SparrowDBStorage.VECTOR_DIMENSIONS) {
      logger.warn(
        `⚠️ findSimilar rejected — dim mismatch: got ${embedding.length}, expected ${SparrowDBStorage.VECTOR_DIMENSIONS}`
      )
      return []
    }
    ...
    try {
      ...
      logger.warn(
        `⚠️ SparrowDB vectorSearch failed: ${e instanceof Error ? e.message : String(e)}`
      )
      return []
    }
    ...
    for (const r of raw) {
      ...
      if (!options.includeFlagged && entry.flag) continue

      // Filter: userId (always required).
      if (entry.userId !== options.userId) continue

      // Filter: contentType (optional).
      if (options.contentType && entry.contentType !== options.contentType) continue

      // Filter: subject (optional).
      if (options.subject !== undefined) {
        const s = entry.metadata?.subject
        if (typeof s !== 'string' || s !== options.subject) continue
      }

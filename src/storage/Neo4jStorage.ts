...
export class Neo4jStorage implements StorageSystem {
  ...
  async resolvePersonId(rawName: string): Promise<string | null> {
    ...
    try {
      ...
      for (const rec of result.records) {
        const name = (rec.get('name') as string || '').toLowerCase()
        const aliases = (rec.get('aliases') as string[]).map(a => a.toLowerCase())
  ...
  async getStats(): Promise<Record<string, any>> {
    ...
    try {
      ...
      const contentTypes = Object.fromEntries(
        contentTypeResult.records.map(r => [r.get('contentType'), r.get('count').toNumber()])
      )
      ...
      const relationshipTypes = Object.fromEntries(
        relTypeResult.records.map(r => [r.get('relType'), r.get('count').toNumber()])
      )
    ...
    try {
      ...
      return result.records.map(record => {
        ...
        return {
          ...
          distance,
          pathTypes: pathRels.map((r: any) => r.type),
          sourceSystem: 'neo4j'
        }
      })
    } finally {
  ...
  private async createSemanticRelationships(session: Session, knowledge: UnifiedKnowledge): Promise<void> {

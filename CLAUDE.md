...
**The gate uses `metadata.subject` as a critical scope filter.** Two writes with the same `subject` get a tight dedup check; two writes that share only a broad topic but no subject won't match in the gate even if their content is similar. Tag high-traffic facts with `metadata.subject` (see preceding section) to make the gate work for you.
